...
class MainPanel extends JPanel implements KeyListener, Runnable, Common {
    ...
    // kich thuoc khung tro choi
    // 20ms/frame = 50fps
    private static final int PERIOD = 20;

    // debug mode
    private static final boolean DEBUG_MODE = true;// chu mau vang o tren

    // map list
    private Map[] maps;
    // current map number
    private int mapNo;// vi tri map cua nhan vat
    ...
    private static Rectangle WND_RECT = new Rectangle(142, 480, 356, 140);// tao 1 cua so chat hinh chu nhat
    ...
    private static final String[] bgmNames = {"castle", "field","forest"};
    // Sound Clip
    ...
    public MainPanel() {
        ...
        // duoi.map la toa do cua map con duoi .event la cac su kien cua doi tuong trong map va loi thoai cua no
        maps[1] = new Map("map/field.map", "event/field.evt", "field", this);
        maps[2] = new Map("map/forest.map", "event/forest.evt", "forest", this);

        mapNo = 0;  // initial map, vi tri xuat hien ban dau

        // create character
        hero = new Character(10, 8, 0, DOWN, 0, maps[mapNo]);
        // cac bien da giai thich trong class character
...
//        loadSound();
    ...
    public void run() {
        long beforeTime, timeDiff, sleepTime;// thoi gian truoc,thoi gian game hoat dong,thoi gian nghi
    ...
    private void checkInput() {
        if (messageWindow.isVisible()) {
            messageWindowCheckInput();// kiem tra xem co an space khong de nextpage or hide
        } else {
    ...
    private void gameRender() {
        // chu yeu la tao nhan vat o giua man hinh hay khong
        if (dbImage == null) {
            // buffer image
        ...
        if (DEBUG_MODE) {
            ...
            dbg.drawString("(" + hero.getX() + "," + hero.getY() + ") ", 4, 32);// vi tri dang o vuong
            dbg.drawString("(" + hero.getPX() + "," + hero.getPY() + ")", 4, 48);// vi tri dang pixal
    ...
    private void heroMove() {// kiem tra xem hero co di chuyen sang map moi hay ko
        if (hero.isMoving()) {
            if (hero.move()) {
                ...
                if (event instanceof MoveEvent) {
                    ...
                    mapNo = m.destMapNo;
                    hero = new Character(m.destX, m.destY, 0, DOWN, 0, maps[mapNo]);// tao mot hero moi o map moi
                    maps[mapNo].addCharacter(hero);
                    midiEngine.play(maps[mapNo].getBgmName());
                }
            }
    ...
    private void characterMove() {
        ...
        Vector<Character> characters = maps[mapNo].getCharacters();// chon che do di chuyen cho linh
        // move each character
