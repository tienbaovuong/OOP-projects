...
public class Map implements Common {
    ...
    // map size (tile) hang va cot
    private int row;
    private int col;

    // map size (pixel)  chieu dai va rong
    private int width;
    private int height;

    // chip set
    private static BufferedImage image;// mot cai buffer de them anh

    // characters in this map
    private Vector<Character> characters = new Vector<Character>();// vector tuong tu mot mang
    // events in this map
    ...
    public Map(String mapFile, String eventFile, String bgmName, MainPanel panel) {
        // file toa do map,file cac event trong map cua object,ten map, panel
        ...
        try {
            ...
            for (int i=0; i<row; i++) {
                ...
                for (int j=0; j<col; j++) {
                    ...
                    int num = charat;
                    map[i][j] = num-48;// chuyen ma ascii
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
